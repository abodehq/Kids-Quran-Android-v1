...
public class FloatingActionsMenu extends ViewGroup {
	...
	private void init(Context context, AttributeSet attributeSet) {
		...
		mIcon = attr.getResourceId(
				R.styleable.FloatingActionsMenu_fab_menu_icon, 0);
		attr.recycle();
		...
		mAddButton = new AddFloatingActionButton(context) {
			...
			Drawable getIconDrawable() {
				...
				final RotatingDrawable rotatingDrawable = new RotatingDrawable(
						getResources().getDrawable(mIcon));
		...
		case EXPAND_DOWN:
			...
			for (int i = mButtonsCount - 1; i >= 0; i--) {
				...
				int childX = addButtonLeft
						+ (mAddButton.getMeasuredWidth() - child
								.getMeasuredWidth()) / 2;
				...
				if (label != null) {
					...
					label.layout(labelLeft, labelTop, labelsRight, labelTop
